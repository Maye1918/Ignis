...
{
    setWindowTitle(QStringLiteral("Ignis - Updater"));
...
void InstallUpdateDialog::install()
{
  ...
  QString extractDirectory = this->temporaryDirectory + QDir::separator() + QStringLiteral("Ignis-Dolphin");
  ...
  if (this->filename.endsWith(QStringLiteral(".zip")))
  {
    ...
    {
      if (!dir.mkdir(QStringLiteral("Ignis-Dolphin")))
      {
